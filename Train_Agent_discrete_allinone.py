...
class A3CAgent:
    def __init__(self, state_size, action_size, env_name):
        ...
        self.hidden1, self.hidden2, self.hidden3 = 1000, 800, 1000
        self.threads = 1 # 48 or 16 or 32 - corresponds to parallel agents
...
class Agent(threading.Thread):
    ...
    def run(self):
        ...
        while episode < EPISODES_train:
            ...
            while True:
                ...
                # for supervised
                df_write = pd.DataFrame(np.array([self.get_usable_observation(state)]))
                df_write.to_csv('state_input.csv', mode='a', header=False)
                
                df_write = pd.DataFrame(np.array([action]))
                df_write.to_csv('chosen_action.csv', mode='a', header=False)
                ...
                # for supervised
                df_write = pd.DataFrame(np.array([reward]))
                df_write.to_csv('reward.csv', mode='a', header=False)
    ...
    def get_action(self, env, state):
        ...
        if roll < epsilon:
            ...
            epsilon = np.max([0.00,epsilon*0.99])
            '''
        ...
        if done_ or sum(((( obs_.rho - 1) )[obs_.rho > 1.00]))>0:
    ...
    def get_usable_observation(self,obs): # penalizing overloaded lines
        ...
        usable_observation = np.hstack((obs_numerical,topo_vect,line_status))
...
if __name__ == '__main__':
    ...
    state_size = 943  # all_observartion
