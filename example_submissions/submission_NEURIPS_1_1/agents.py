...
loaded = np.load(os.path.split(os.path.realpath(__file__))[0]+'/'+ 'actions_array_1502.npz')
actions_array = (loaded['actions_array_useful'])  # this has 157 actions
...
class MyAgent(BaseAgent):
    ...
    def __init__(self, action_space):
        ...
        self.state_size = 767
        self.action_size = 1502  
        self.hidden1, self.hidden2 = 1000, 1000
    ...
    def est_reward_update(self,obs,rw,done): # penalizing overloaded lines
        ...
        rw_0 = rw - 10 * sum(((( state_obs.rho - 1) )[
                        state_obs.rho > 1])) if not done else -999
        return rw_0
    '''
    def est_reward_update(self,obs,rw,done): # penalizing overloaded lines
        #obs = observation_space.array_to_observation(obs) if not done else 0
    ...
    def get_observation_vector(self,OBS_0):
        ...
        numeric_features = np.hstack((prod_p, prod_q, load_p, load_q, OBS_0.rho))
        
        #topo_vect = OBS_0.topo_vect-1
        line_status = 1 - OBS_0.line_status
        
        selected_features = np.hstack((numeric_features, graph_features,line_status))
