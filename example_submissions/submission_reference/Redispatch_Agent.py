...
class MyRedispatch(BaseAgent):
    ...
    def act(self, observation, done=False):
        ...
        """
        self.redispatch_action = self._get_tested_action(self, observation)
        if len(self.redispatch_action) > 1:
            all_rewards = np.full(shape=len(self.redispatch_action), fill_value=np.NaN, dtype=dt_float)
            for i, action in enumerate(self.tested_action):
                simul_obs, simul_reward, simul_has_error, simul_info = observation.simulate(action)
                all_rewards[i] = simul_reward
            
            if np.max(all_rewards) > all_rewards[0] + 10 :
    ...
    def _get_tested_action(self, observation):
        ...
        res += self.action_space.get_all_unitary_redispatch(self.action_space)
        self.redispatch_action = res
        
        # my added for generation redispatch
        #(gen_idx, ramp)
        #action = self.action_space({"redispatch": [(gen_idx, ramp)]})
        #res.append(action)
        
        return self.redispatch_action
