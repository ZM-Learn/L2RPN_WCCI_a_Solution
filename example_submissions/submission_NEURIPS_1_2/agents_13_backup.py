...
loaded2 = np.load('norm_range.npz')
norm_range = loaded2['norm_range']
norm_range = norm_range[1:]

loaded2 = np.load('norm_min.npz')
...
class MyAgent(BaseAgent):
    ...
    def est_reward_update(self,obs,rw,done): # penalizing overloaded lines
        ...
        state_obs = obs
        if not done:
            rw_0 = rw/10 - 2 * sum(((( state_obs.rho - 0.99) )[state_obs.rho > 1])) - 0.1 * sum(((( state_obs.rho - 0.8) )[state_obs.rho > 0.8]))  
            if rw_0<-5:
                rw_0 = -5
        else:
            rw_0 = -10
